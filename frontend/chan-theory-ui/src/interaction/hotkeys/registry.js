...
export class HotkeyService {                                 // 定义热键服务类
  constructor(defaultKeymap) {                               // 构造函数
    ...
    window.addEventListener("keydown", this._onKeydown, {    // 注册键盘监听（捕获阶段）
      capture: true,
    });
  }                                                          // 结束构造
  ...
  get keymap() {                                             // 计算属性：合并后的键位表
    const merged = {};                                       // 合并结果
    const scopes = new Set([                                 // 所有 scope（默认 + 覆盖）
      ...Object.keys(this.defaultKeymap),
      ...Object.keys(this.userOverrides || {}),
    ]);                                                      // 结束集合
    scopes.forEach((s) => {                                  // 遍历 scope
      merged[s] = Object.assign(                             // 合并（默认优先，覆盖后应用）
        {},
  ...
  registerHandlers(scope, map) {                             // 注册命令处理器
    this.handlers[scope] = Object.assign(                    // 覆盖追加
      {},
      this.handlers[scope] || {},
      map || {}
    );
  ...
  unregisterHandlers(scope) {                                // 注销某 scope 处理器
    delete this.handlers[scope];                             // 删除映射
  }                                                          // 结束注销

  pushScope(scope) {                                         // 压入作用域
    const s = this.scopeStack.value.slice();                 // 拷贝当前栈
    s.push(scope);                                           // 追加
    this.scopeStack.value = s;                               // 写回
  }                                                          // 结束 pushScope

  popScope(scope) {                                          // 弹出作用域
    if (!scope) {                                            // 无参数：弹最顶层（保留 global）
  ...
  setBinding(scope, command, combo) {                        // 设置单条覆盖（命令→组合）
    const km = this._invert(this.keymap[scope] || {});       // 反转映射（combo->cmd → cmd->combo）
    Object.keys(this.userOverrides[scope] || {}).forEach((c) => { // 删除旧绑定
      if ((this.userOverrides[scope] || {})[c] === command)  // 同命令
  ...
  setUserOverrides(overrides) {                              // 设置整表覆盖
    this.userOverrides = overrides || {};                    // 覆盖
  }                                                          // 结束 setUserOverrides

  getBindings(scope) {                                       // 获取“命令 → 组合”视图
    const merged = this.keymap[scope] || {};                 // 合并映射
    return this._invert(merged);                             // 反转（返回 cmd->combo）
  }                                                          // 结束 getBindings

  _invert(obj) {                                             // 反转 {a:b} → {b:a}
    const out = {};                                          // 结果
    Object.keys(obj || {}).forEach((k) => (out[obj[k]] = k));// 遍历反转
    return out;                                              // 返回
  }                                                          // 结束 _invert

  focusNextPrev(dir = +1) {                                  // 默认行为：在输入可聚焦元素间切换
    const candidates = Array.from(                           // 查找可见可用输入控件
  ...
  _onKeydown(e) {                                            // 全局 keydown 处理
    ...
    const inputWhitelist = new Set([                         // 输入环境白名单组合
      "Escape",                                              // 取消
      "Ctrl+Enter",                                          // 确认
      "Meta+Enter",                                          // 确认（Mac）
      "Tab",                                                 // 下一个输入
    ...
    for (const scope of stack) {                             // 逐层匹配
      ...
      if (!handler) {                                        // 没有处理器 → 尝试内置
        if (cmd === "focusNextField") {                      // 内置：下一个输入
          e.preventDefault();                                 // 阻止默认
          this.focusNextPrev(+1);                             // 跳转
          return;                                             // 返回
        }
        if (cmd === "focusPrevField") {                      // 内置：上一个输入
          e.preventDefault();                                 // 阻止默认
          this.focusNextPrev(-1);                             // 跳转
          return;                                             // 返回
        }
        if (cmd === "openHotkeySettings" || cmd === "openHotkeyHelp") { // 内置：打开设置/帮助
          e.preventDefault();                                 // 阻止默认
