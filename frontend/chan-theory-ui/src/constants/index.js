...
// ==============================                                           // 分隔注释
// 说明：集中管理“前端可见设置项”与“设置性质的不可见控制参数”的默认值。                   // 文件概述说明
...
// - 本次改动：                                                                  // 本轮改动摘要
//   1) 取消 YTD 预设，保留 ALL，与其他窗宽并列，遵循“ALL=当前已加载序列的总根数 n”的语义。   // 变更点 1
//   2) 新增 preset→barsCount 近似映射工具（presetToBars / minuteBarsPerDay），           // 变更点 2
//      统一将预设映射为“原始 K 根数”。                                                  // 变更点 2（续）
//   3) 新增 pickPresetByBarsCountDown（“向下”就近高亮），当滚轮缩放落在两个预设之间时，      // 变更点 3
//      选择较小者高亮。                                                               // 变更点 3（续）
//   4) 仍保留 WINDOW_PRESETS（不含 YTD），作为 UI 渲染按钮的唯一来源。                    // 变更点 4
//   5) 关键修改：所有非整数 bars 一律“向上取整”，bars 下限为 1（宁多不少）。                 // 变更点 5（新增）
// ==============================                                           // 分隔注释
...
export const DEFAULT_KLINE_STYLE = {
  // 主图 K 线样式默认
  barPercent: 100, // 柱宽百分比（1~100）
  upColor: "#f56c6c", // 阳线颜色
  downColor: "#26a69a", // 阴线颜色
  subType: "candlestick", // 子类型：蜡烛图（或 "bar" HL柱）
}; // 对象结束
